...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    // SECURE CONCURRENCY: Run inside a Prisma transaction with a row lock to prevent race conditions!
    const newToken = await prisma.$transaction(async (tx) => {
      // 1. Lock the Doctor row to serialize all check-ins under the same doctor
